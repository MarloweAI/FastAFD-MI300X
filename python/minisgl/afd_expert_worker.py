    ...
    *,
    ...
    def launch_expert(layer: int, mb: int) -> None:
        ...
        with torch.cuda.stream(engine_stream):
            ...
            # A replay is ONE host call in place of ~20 eager ops (doc 38: 782 launches per step,
            # ~10.5 ms of host dispatch against 2.76 ms of device work). Keyed on the exact row
            # count; any count without a captured graph runs eager, which stays correct.
            rows = int(disp.hidden_states.shape[0])
            if rows > 0 and expert_graphs is not None and expert_graphs.usable(rows, mb):
                expert_out = expert_graphs.replay(
                    n=rows, mb=mb, layer=layer, dispatch_output=disp
                )
            else:
    ...
    #
    # step_timer is host-side and does not synchronize -- see profiling.py. Under the serial
    # afd_num_mb=1 schedule the collectives block the host, so these buckets separate real
    # expert work from time spent blocked on the AG stage, which is what doc 28 §8 could only
    # bound. No-op with profiling off.
    from minisgl.profiling import step_timer
...
class AfdExpertWorker(BaseAfdWorker):
    ...
    def _run_eg_pipeline_eager(self, plan: AfdEGStepPlan, bucket: int) -> None:
        ...
        """
        # Profiled so the EG rank's own kernel time is measurable. Without this, an AG-side
        # profile cannot distinguish "EG is slow" from "EG is idle waiting for AG" -- the two
        # look identical from the AG rank, and doc 26 §3 guessed wrong for exactly that reason.
        from minisgl.profiling import profile_region, step_flush, sync_scan

        _t_step = time.perf_counter()
        _phase = getattr(plan, "phase", "decode")
        with sync_scan(tag=f"afd_eg_{_phase}"), profile_region(
            tag="afd_eg", phase=getattr(plan, "phase", "decode"),
            n_tok=int(bucket), device=self.runtime_state.stream.device,
            rank=f"mlp{self.tp_rank}",
            extra={"num_mb": int(plan.num_mb), "bucket": int(bucket)},
        ):
            self._run_eg_pipeline_eager_inner(plan, bucket)
        step_flush(
            tag="afd_eg", phase=_phase, rank=f"mlp{self.tp_rank}",
            total_ms=(time.perf_counter() - _t_step) * 1e3,
            extra={"num_mb": int(plan.num_mb), "bucket": int(bucket)},
        )

    def _run_eg_pipeline_eager_inner(self, plan: AfdEGStepPlan, bucket: int) -> None:
        ...
        _run_eg_deepep_pipeline_body(
            ...
            expert_graphs=getattr(self, "_eg_expert_graphs", None),
        )
        for stream in lane_streams:
            self.runtime_state.stream.wait_stream(stream)
