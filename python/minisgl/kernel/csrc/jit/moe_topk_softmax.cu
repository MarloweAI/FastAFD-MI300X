...
inline constexpr int kWarpSize = static_cast<int>(device::kWaveThreads);
...
__device__ __forceinline__ auto shfl_xor_width(
    device::warp_mask_t mask, T value, int lane_mask, int width) -> T {
...
__launch_bounds__(WARPS_PER_CTA * kWarpSize) __global__ void topk_gating_softmax_kernel(
  ...
  // Do NOT `return` here. ROWS_PER_WARP is kWarpSize/THREADS_PER_ROW, which is 1
  // on a 32-lane warp but 2+ on wave64 (e.g. 128 experts -> THREADS_PER_ROW=32,
  // ROWS_PER_WARP=2). With more than one row per wave the bound check is not
  // wave-uniform: for an odd num_rows the upper half-wave would exit while the
  // lower half still executes the __shfl_xor_sync reductions below with a
  // full-participation mask. On gfx942 that faults with
  // HSA_STATUS_ERROR_EXCEPTION / hipErrorLaunchFailure. Keep every lane resident
  // through the cross-lane reductions and gate only the stores on `row_valid`.
  const bool row_valid = thread_row < num_rows;
  const int safe_row = row_valid ? thread_row : 0;

  const float* thread_row_ptr = input + safe_row * ELTS_PER_ROW;
...
#pragma unroll
  for (int mask = THREADS_PER_ROW / 2; mask > 0; mask /= 2) {
    thread_max = max(
        thread_max,
        shfl_xor_width(device::kFullWarpMask, thread_max, mask, THREADS_PER_ROW));
  }
...
#pragma unroll
  for (int mask = THREADS_PER_ROW / 2; mask > 0; mask /= 2) {
    row_sum += shfl_xor_width(device::kFullWarpMask, row_sum, mask, THREADS_PER_ROW);
  }
...
#pragma unroll
    for (int mask = THREADS_PER_ROW / 2; mask > 0; mask /= 2) {
      const float other_max =
          shfl_xor_width(device::kFullWarpMask, max_val, mask, THREADS_PER_ROW);
      const int other_expert =
          shfl_xor_width(device::kFullWarpMask, expert, mask, THREADS_PER_ROW);
    ...
    if (thread_group_idx == 0 && row_valid) {
  ...
  if (renormalize && thread_group_idx == 0 && row_valid) {
...
__launch_bounds__(WARPS_PER_CTA * kWarpSize) __global__ void topk_gating_softmax_group_local_kernel(
  ...
  // Neither bound check may `return` on wave64: ROWS_PER_WARP > 1 there, so both
  // are per-row and therefore not wave-uniform, and the __shfl_xor_sync
  // reductions further down require full participation. See the longer note in
  // topk_gating_softmax_kernel. Every lane stays resident; stores are gated.
  const bool row_in_range = thread_row < num_rows;
  const int safe_row = row_in_range ? thread_row : 0;

  bool row_is_valid = row_in_range;
  if (row_in_range && valid_count_size != 0) {
    const int valid_count = min(static_cast<int>(valid_token_count[0]), num_rows);
    row_is_valid = thread_row < valid_count;
  }
  // In-range rows beyond valid_count still get their outputs cleared, exactly as
  // the original early-return path did.
  if (row_in_range && !row_is_valid && thread_group_idx == 0) {
#pragma unroll
    for (int k_idx = 0; k_idx < topk; ++k_idx) {
      const int idx = topk * thread_row + k_idx;
      output[idx] = 0.0f;
  ...
  const InputT* thread_row_ptr = input + safe_row * ELTS_PER_ROW;
  const int first_elt_read_by_thread = thread_group_idx * ELTS_PER_LDG;
...
#pragma unroll
    ...
    if (thread_group_idx == 0 && row_is_valid) {
      const int idx = topk * thread_row + k_idx;
      output[idx] = max_val;
  ...
  if (renormalize && thread_group_idx == 0 && row_is_valid) {
...
topk_gating_kernel_v2(
  ...
  const int thread_row_in_warp = threadIdx.x / THREADS_PER_ROW;
  const int thread_row = warp_base_row + thread_row_in_warp;
  // Not wave-uniform once ROWS_PER_WARP > 1 (wave64); see the note in
  // topk_gating_softmax_kernel. Stay resident through the reductions, gate stores.
  const bool row_valid = thread_row < num_rows;
  const int safe_row = row_valid ? thread_row : 0;
...
#pragma unroll
    for (int mask = THREADS_PER_ROW / 2; mask > 0; mask /= 2) {
      const float other_score =
          shfl_xor_width(device::kFullWarpMask, max_score, mask, THREADS_PER_ROW);
...
#pragma unroll 8
  ...
  if (staged_output != nullptr && thread_group_idx == 0 && row_valid) {
#pragma unroll 8
    for (int k_idx = 0; k_idx < topk; ++k_idx) {
      const int idx = topk * thread_row + k_idx;
...
topk_gating_kernel_v2_padded(
  ...
  // Safe to `return` here, unlike in the other gating kernels: ROWS_PER_WARP is 1
  // and thread_row depends only on threadIdx.y, so every lane of a wave shares the
  // same row and the bound check is wave-uniform even on wave64.
...
#pragma unroll
    for (int mask = kWarpSize / 2; mask > 0; mask /= 2) {
      thread_max = max(thread_max, shfl_xor_width(device::kFullWarpMask, thread_max, mask, kWarpSize));
    }
    float row_sum = 0.0f;
#pragma unroll
    for (int ii = 0; ii < VPT; ++ii) {
...
#pragma unroll
    for (int mask = kWarpSize / 2; mask > 0; mask /= 2) {
      row_sum += shfl_xor_width(device::kFullWarpMask, row_sum, mask, kWarpSize);
    }
    const float inv_sum = __frcp_rn(row_sum);
#pragma unroll
    for (int ii = 0; ii < VPT; ++ii) row_chunk[ii] *= inv_sum;
...
#pragma unroll
    for (int mask = kWarpSize / 2; mask > 0; mask /= 2) {
      const float other_score = shfl_xor_width(device::kFullWarpMask, max_score, mask, kWarpSize);
      const float other_val = shfl_xor_width(device::kFullWarpMask, max_val, mask, kWarpSize);
      const int other_expert = shfl_xor_width(device::kFullWarpMask, expert, mask, kWarpSize);
      if (other_score > max_score ||
          (other_score == max_score && other_expert < expert)) {
        max_score = other_score;
        max_val = other_val;
...
void topk_gating_v2_padded_launcher_helper(
    ...
    int num_tokens, int topk, bool renormalize,
    cudaStream_t stream) {
  // The padded v2 kernel maps one full warp to one row and static_asserts
  // THREADS_PER_ROW == kWarpSize, so VPT must follow the wave width rather than
  // being hardcoded: 256/32 = 8 on CUDA (unchanged), 256/64 = 4 on wave64.
  static constexpr int VPT = PADDED_EXPERTS / kWarpSize;
  static_assert(VPT >= 1 && (VPT & (VPT - 1)) == 0,
                "PADDED_EXPERTS must be a power-of-2 multiple of the wave width");
