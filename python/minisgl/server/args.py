...
def parse_args(args: List[str], run_shell: bool = False) -> Tuple[ServerArgs, bool]:
    ...
    parser.add_argument(
        ...
        type=str,
        default="auto",
        choices=["auto", "fp8", "fp8_v", "fp8_e4m3fn", "fp8_e5m2"],
        help=(
            "KV cache storage precision, independent of --dtype. 'auto' matches --dtype. "
            "'fp8' halves KV bytes and doubles the pages that fit; on gfx942 it means the "
            "native e4m3fnuz. Decode at large batch x long context is KV-bandwidth bound, so "
            "this is a throughput knob as well as a capacity one. NOTE: plain 'fp8' fails the "
            "T-10 correctness gate on gpt-oss because K's quantisation error is exponentiated "
            "by the softmax; 'fp8_v' quantises V only (25% saving, bounded error). See "
            "dev_log/gpt_oss_120b/21_fp8_kv_cache.md."
        ),
    )
    ...
    if (dtype_str := kwargs["dtype"]) == "auto":
        ...
        dtype_str = getattr(hf_cfg, "dtype", None) or getattr(hf_cfg, "torch_dtype", None)
        if dtype_str is None:
            # Some checkpoints declare no dtype at all. gpt-oss-120b is one: its
            # `quantization_config` describes the MXFP4 experts, and every unquantised
            # tensor (attention, router, embeddings, lm_head) is BF16 -- confirmed by
            # reading the safetensors, which hold only BF16 and U8.
            # This used to fall through as None and surface ~200 lines later as
            # "'NoneType' object has no attribute 'itemsize'" inside TP setup.
            dtype_str = "bfloat16"
    if isinstance(dtype_str, str):
        if dtype_str not in DTYPE_MAP:
            raise ValueError(
                f"unsupported dtype {dtype_str!r}; expected one of {sorted(DTYPE_MAP)}"
            )
        kwargs["dtype"] = DTYPE_MAP[dtype_str]
    elif isinstance(dtype_str, torch.dtype):
        kwargs["dtype"] = dtype_str
    else:
        raise TypeError(f"could not resolve a dtype (got {dtype_str!r})")
    ...
    if kwargs["mode"] == "afd-serve":
        ...
        if kwargs["afd_moe_runner_backend"] == "auto":
            # NB: no local `import torch` here -- args.py imports torch at module
            # scope, and a function-local import would rebind the name for the whole
            # function body, breaking the DTYPE_MAP lookup further down.
            if torch.version.hip is not None:
                # gfx942: DeepGEMM is SM90/SM100-only, so deep_gemm is not an option
                # at all. The deadlock noted above is specific to DeepEP's comm
                # contract; the `rccl` M2N transport used on ROCm has no cross-role
                # barrier, and moe/m2n_permute.py supplies the layout bridge the
                # Triton runner needs. See dev_log/qwen/12_afd_wireup.md.
                kwargs["afd_moe_runner_backend"] = "triton"
            else:
