...
_RUNTIME_ENV_KEYS = (
    ...
    "NCCL_DEBUG_SUBSYS",
    "NCCL_SOCKET_IFNAME",
    # The M2N counts exchange is a gloo (CPU) all_to_all, so gloo's interface choice is
    # on the critical path and must be pinned the same way NCCL's is. Unset, gloo
    # auto-selects, and on a host carrying k8s/docker interfaces (flannel.1, cni0,
    # docker0) it can pick one that cannot route between the actors -- which presents as
    # a send that never completes and a 300 s timeout, not as a connection error.
    "GLOO_SOCKET_IFNAME",
    "TP_SOCKET_IFNAME",
    "NCCL_IB_HCA",
    "NCCL_IB_GID_INDEX",
    ...
    "TORCH_USE_CUDA_DSA",
    # ROCm debugging. An async HIP fault is reported at the next synchronising call, not at its
    # site -- doc 43 burned two hypotheses for want of these. AMD_SERIALIZE_KERNEL=3 serialises
    # launches so the fault lands on the guilty kernel; the DSA flags enable device-side asserts.
    # They must reach the ACTORS, where the faulting kernel actually runs.
    "AMD_SERIALIZE_KERNEL",
    "TORCH_USE_HIP_DSA",
    "HIP_LAUNCH_BLOCKING",
    # Profiling knobs must be forwarded explicitly. They reached the actors before
    # only because the raylet happened to inherit the driver's environment -- which
    # holds when the driver starts Ray and silently fails against an already-running
    # cluster. That is one cause of the empty AFD profile directories in doc 27 §3.3.
    "MINISGL_PROFILE_DIR",
    "MINISGL_PROFILE_PHASE",
    "MINISGL_PROFILE_SKIP",
    "MINISGL_PROFILE_STEPS",
    "MINISGL_PROFILE_MIN_TOK",
    "MINISGL_PROFILE_TIME_ONLY",
    "MINISGL_AFD_HOST_VISIBLE_DEVICES",
    # Without this the kill-switch is unreachable from the launcher: the driver would honour
    # it and the actors -- where dispatch actually runs -- would not, so an A/B would silently
    # compare the fused path against itself.
    "MINISGL_M2N_FUSE_DISPATCH",
    "MINISGL_M2N_FIXED_SHAPE",
    "MINISGL_M2N_FIXED_SHAPE_MAX_BYTES",
    "MINISGL_M2N_DRAIN_SPLIT",
    "MINISGL_M2N_COMBINE_DRAIN",
    "MINISGL_M2N_FUSED_PLAN",
    "MINISGL_M2N_FUSED_GROUP",
    "MINISGL_AFD_AG_ROUTE_GRAPH",
    "MINISGL_AFD_AG_ROUTE_GRAPH_MAX_BUCKETS",
    "MINISGL_AFD_AG_ROUTE_GRAPH_BS",
    "MINISGL_AFD_AG_ATTN_GRAPH",
    "MINISGL_AFD_AG_ATTN_GRAPH_MAX_BUCKETS",
    "MINISGL_AFD_EG_EXPERT_GRAPH",
    "MINISGL_AFD_EG_EXPERT_GRAPH_ROWS",
    "MINISGL_AFD_EG_EXPERT_GRAPH_MAX_BUCKETS",
    "MINISGL_SYNC_SCAN",
    "MINISGL_SYNC_SCAN_FILE",
)
...
def runtime_env(
    *,
    ...
    for key in _RUNTIME_ENV_KEYS:
        ...
        if value:
            env_vars[key] = value
    # Record the DRIVER's GPU slice under a name Ray does not touch. Ray overwrites
    # the per-actor visibility variable with just that actor's card, so a worker that
    # needs the whole slice (megamoe_m2n exchanges device ordinals across ranks) cannot
    # recover it from the standard variable. Forwarding the standard name instead would
    # race Ray's own assignment. See AfdWorkerBase.__init__.
    host_slice = os.environ.get("HIP_VISIBLE_DEVICES") or os.environ.get(
        "CUDA_VISIBLE_DEVICES"
    )
    if host_slice:
        env_vars["MINISGL_AFD_HOST_VISIBLE_DEVICES"] = host_slice
...
def _supports_decode_graph_capture(
    ...
    if decode_backend == "triton_decode":
        # dev_log/qwen/16_hip_graphs.md. Without this entry the whitelist silently
        # returned False, `decode_graph_bs` came out empty, and AFD ran fully eager
        # while still reporting attention_decode_graph=True -- so the logs looked
        # like graphs were on.
        return int(page_size) == 1
    return False
...
def normalize_attention_backend_page_size(
    ...
    if backend == "auto":
        ...
        if torch.version.hip is not None:
            # `trtllm` (TensorRT-LLM) and `fi` (FlashInfer) are both NVIDIA-only.
            # Mirrors engine.py:_adjust_config, including the prefill/decode split:
            # decode is HIP-graph capturable, prefill is 2.3-2.5x faster than torch_ref.
