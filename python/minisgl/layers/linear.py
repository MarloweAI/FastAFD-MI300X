...
class _LinearTPImpl(BaseOP):
    ...
    def __init__(
        ...
        # Row-parallel layers (LinearOProj, LinearRowParallel) must add their bias AFTER
        # the all-reduce: each rank holds the full-width bias, so folding it into the GEMM
        # would contribute it once per rank and scale it by tp_size. Those subclasses set
        # this, and `_fused_bias` then keeps it out of the matmul.
        self._defer_bias = False

    @property
    def _fused_bias(self) -> torch.Tensor | None:
        """The bias to fold into the GEMM, or None when a subclass adds it post-reduce."""
        return None if self._defer_bias else self.bias
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return F.linear(x, self.weight, self._fused_bias)
    ...
    def _forward_bf16_dequant(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return F.linear(x, w, self._fused_bias)
    ...
    def _forward_fp8(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self._fused_bias is not None:
            output = output + self._fused_bias
...
class LinearOProj(_LinearTPImpl):
    def __init__(
        ...
        self._use_bf16_cublas_for_fp8 = False
        self._defer_bias = has_bias and tp_info.size > 1

    def forward(self, x: torch.Tensor, skip_allreduce: bool = False) -> torch.Tensor:
        if self._defer_bias and skip_allreduce:
            # The caller owns the reduce, so it would have to own the bias add too.
            # Refuse rather than silently returning a bias-free partial sum.
            raise NotImplementedError(
                "LinearOProj: skip_allreduce with a bias is not supported at tp_size>1; "
                "the bias must be added after the reduce that the caller performs."
            )
        y = super().forward(x)
