...
# Short alias for the sub-step timer. It is a no-op unless MINISGL_PROFILE_DIR is set, and it
# must stay cheap: launch_compute runs 36x per step.
...
def _run_ag_pipeline_body(
    *,
    ...
    route_graphs: Any = None,
    attn_graphs: Any = None,
) -> None:
    ...
    def _attended(layer: int, mb: int, hidden: torch.Tensor, residual):
        """forward_attention(), replayed from a warmup-captured graph when one matches.

        REPLAY ONLY. `prepare_step` below must already have refilled the captured metadata
        buffers from THIS step's batch -- otherwise the graph replays against the warmup KV
        layout and silently produces wrong output, which is the failure mode the whole-step
        runner's `replay` documents.
        """
        if attn_graphs is None or torch.cuda.is_current_stream_capturing():
            return model.forward_attention(layer, hidden, residual)
        bs = int(hidden.shape[0])
        if attn_graphs.usable(bs, mb):
            return attn_graphs.replay(
                bs=bs, mb=mb, layer=layer, hidden=hidden, residual=residual
            )
        return model.forward_attention(layer, hidden, residual)

    def _routed(layer: int, mb: int, hidden: torch.Tensor):
        """route(), replayed from a warmup-captured graph when one matches this shape.

        REPLAY ONLY -- capture never happens here. Capturing inline on a live step works at
        bs=1 and corrupts under concurrency, because M2N work is in flight on the lane streams
        while a 36-graph capture holds the engine stream; it failed with "operation failed due
        to a previous error during capture" and tripped the RCCL heartbeat monitor. Capture is
        done once at warmup instead. A batch size with no captured bucket just runs eager.
        """
        if route_graphs is None or torch.cuda.is_current_stream_capturing():
            return model.route(layer, hidden)
        bs = int(hidden.shape[0])
        if route_graphs.usable(bs, mb):
            return route_graphs.replay(bs=bs, mb=mb, layer=layer, hidden=hidden)
        return model.route(layer, hidden)
    ...
    def launch_compute(layer: int, mb: int):
        ...
        try:
            with ctx.forward_batch(state.subs[mb]):
                with torch.cuda.stream(engine_stream):
                    if layer > 0:
                        with _sub("c.wait_event"):
                            engine_stream.wait_event(state.done_events[layer - 1][mb])
                    with _sub("c.attn"):
                        st = _attended(
                            layer, mb, state.hidden[mb], state.residual[mb]
                        )
                    state.hidden[mb], state.residual[mb] = (
                        st.hidden_states,
                        st.residual,
                    ...
                    if state.use_megamoe and hasattr(model, "route_for_megamoe"):
                        ...
                        with _sub("c.route"):
                            topk = _routed(layer, mb, state.hidden[mb])
    ...
    #
    # Profiled as one region so the step's cost is DECOMPOSED rather than inferred. Not
    # capturable-safe by construction, but profile_region is a no-op unless
    # MINISGL_PROFILE_DIR is set, and capture runs never set it.
    #
    # Every argument below must be safe to evaluate on the hot path even with profiling
    # OFF: Python evaluates the argument list BEFORE profile_region can no-op, so a bad
    # expression here is an unconditional crash in the serving path, not a dead profiling
    # branch. The EG hook shipped with `self.rank` (no such attribute) and took down every
    # expert rank on every step until it was found -- which is what doc 27 §3.3 recorded as
    # "three AFD servers died", wrongly blamed on the harness.
    from minisgl.profiling import profile_region, step_flush, step_timer, sync_scan

    _n_tok = sum(int(h.shape[0]) for h in state.hidden if h is not None)
    _first = next((h for h in state.hidden if h is not None), None)
    _t_step = time.perf_counter()
    # `state` carries no `rank`, so the original hook's getattr(state, "rank", 0) labelled
    # BOTH AG ranks as rank 0 -- so they raced on one output path and produced concatenated,
    # unparseable JSON ("Extra data: line 27"). Use the real distributed rank.
    try:
        _rank = torch.distributed.get_rank() if torch.distributed.is_initialized() else 0
    except Exception:
        _rank = getattr(state, "rank", 0)
    with sync_scan(tag=f"afd_ag_{phase}"), profile_region(
        tag="afd_ag", phase=phase, n_tok=_n_tok,
        device=(
            None
            if _first is None or torch.cuda.is_current_stream_capturing()
            else _first.device
        ),
        rank=_rank,
        extra={"num_mb": state.num_mb, "num_layers": num_layers},
    ):
        # Once per step, never per layer: batch.attn_metadata is per-batch
        # (triton_decode.forward reads it), so one refill serves all layers' graphs.
        if attn_graphs is not None:
            for _mb in range(state.num_mb):
                _h = state.hidden[_mb]
                if _h is not None and attn_graphs.usable(int(_h.shape[0]), _mb):
                    attn_graphs.prepare_step(
                        bs=int(_h.shape[0]), mb=_mb, batch=state.subs[_mb]
                    )
        for mb in range(state.num_mb):
            with step_timer("compute"):
                packet = launch_compute(0, mb)
            with step_timer("dispatch"):
                launch_dispatch(0, mb, packet)
            if retire_async_objects:
                with step_timer("retire"):
                    retire()
        for layer in range(num_layers):
            for mb in range(state.num_mb):
                with step_timer("combine"):
                ...
                if retire_async_objects:
                    with step_timer("retire"):
                        retire()
        if retire_async_objects:
            with step_timer("retire_force"):
                retire(force=True)
        for mb in range(state.num_mb):
            with step_timer("wait_done_event"):
                engine_stream.wait_event(state.done_events[num_layers - 1][mb])
    step_flush(
        tag="afd_ag", phase=phase, rank=_rank,
        total_ms=(time.perf_counter() - _t_step) * 1e3,
        extra={"num_mb": state.num_mb, "num_layers": num_layers, "n_tok": _n_tok},
    )
    ...
    def _run_ag_forward_eager(self, batch, plan: AfdAGStepPlan, model, adapters, ctx):
        ...
        with nvtx_range("AFD_AG_PipelineMB"):
            _run_ag_pipeline_body(
                ...
                # Decode only. Prefill shapes vary per request, so capturing there would
                # thrash one graph set per token count for no benefit -- prefill tensors are
                # large enough that launch overhead already amortises (doc 28 §13.4).
                route_graphs=(
                    self._ag_route_graphs if str(plan.phase) == "decode" else None
                ),
                attn_graphs=(
                    self._ag_attn_graphs if str(plan.phase) == "decode" else None
                ),
            )
